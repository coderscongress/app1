...
const ThumbnailViewer = ({ pdfList }) => {
  ...
  const [blockToggleStates, setBlockToggleStates] = useState({});
  ...
  const handleCrop = (blockIndex) => {
    setBlockToggleStates(prevStates => {
      ...
      newStates[blockIndex] = !prevStates[blockIndex];
      return newStates;
    });
    setToggledSetIndex(prevIndex => (prevIndex === blockIndex ? null : blockIndex));
  };
  ...
  const renderBlocksOfThree = () => {
    ...
    for (let i = startIdx; i < endIdx; i += 3) {
      const blockIndex = i; // Index of the first item in the set of three
      const block = pdfList.slice(i, i + 3).map((item, index) => {
        const currentIndex = i + index;
        const isToggled = blockToggleStates[blockIndex];
        ...
        return (
          <div key={index} style={{ margin: '5px' }}>
            ...
            <button 
              ...
              style={{
                borderRadius: '15px', 
                background: isToggled ? 'black' : 'navy', 
                color: 'white', 
                fontSize: '20px', 
                width: '120px', 
                height: '30px'
              ...
              type="button" 
              onClick={() => handleCrop(blockIndex)}
            >
              {isToggled ? 'Hide Book' : 'View Book'}
            </button>
            {toggledSetIndex === blockIndex && (
              <DisplayCroppedPageB
                pdfList={pdfList}
      ...
      blocks.push(
        <div key={blockIndex} style={{ display: 'flex', justifyContent: 'space-evenly', marginBottom: '5px' }}>
          {block}
        </div>
      );
    }
